...
import { useGetCurrentUser, useListAlerts } from "@workspace/api-client-react";
              ...
              <div className="flex items-center gap-1 bg-muted/50 p-1 rounded-md border border-border/50">
                {(["light", "dark", "system"] as const).map((t) => (
                  <button
                    key={t}
                    onClick={() => setTheme(t)}
                    className={`flex-1 text-xs py-1 rounded capitalize transition-colors ${theme === t ? 'bg-background shadow-sm text-foreground font-medium' : 'text-muted-foreground hover:text-foreground'}`}
                  >
                    {t}
                  </button>
                ))}
