...
import { oauthTokensTable, campaignsTable } from "@workspace/db";
  ...
  try {
    ...
    const tokens = await db
      .select({
        ...
        accountName: oauthTokensTable.accountName,
        updatedAt: oauthTokensTable.updatedAt,
      })
      .from(oauthTokensTable)
      .where(eq(oauthTokensTable.userId, userId));

    const status = Object.entries(PLATFORM_CONFIG).map(([platform, cfg]) => {
      ...
      return {
        ...
        needsConfig: !process.env[cfg.clientIdEnv],
      };
    });
...
router.get("/oauth/initiate/:platform", requireAuth, (req, res) => {
  ...
  if (!clientId) {
    res.status(400).json({
      error: `${cfg.clientIdEnv} environment variable is not configured. Add it in Secrets to enable this integration.`,
      needsConfig: true,
...
// GET /oauth/callback/:platform (no auth — browser arrives here after OAuth redirect)
router.get("/oauth/callback/:platform", async (req, res) => {
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      req.log.error({ platform, status: tokenRes.status, errText }, "Token exchange failed");
    ...
    const [existing] = await db
      ...
      .where(
        and(
          ...
          eq(oauthTokensTable.platform, platform),
        ),
      );
    ...
    if (existing) {
      ...
      await db
        ...
        .values({ userId, platform, accessToken, refreshToken, expiresAt, scope });
    }

    // Kick off background sync (non-blocking)
    syncPlatformData(userId, platform, accessToken).catch((e: unknown) => {
      req.log.error({ err: e, platform }, "Background sync failed");
    });

    res.redirect(`${baseUrl}/integrations?connected=${platform}`);
  } catch (err) {
    req.log.error({ err, platform }, "OAuth callback failed");
...
async function syncPlatformData(
  ...
  switch (platform) {
    case "google":
      await syncGoogleAds(userId, accessToken);
      break;
    case "meta":
      await syncMetaAds(userId, accessToken);
      break;
    case "linkedin":
      await syncLinkedInAds(userId, accessToken);
      break;
    case "microsoft":
      await syncMicrosoftAds(userId, accessToken);
      break;
  }
}
...
async function upsertCampaign(
  ...
  const [existing] = await db
    ...
    .from(campaignsTable)
    .where(
      and(eq(campaignsTable.userId, userId), eq(campaignsTable.externalId, externalId)),
    );
...
async function syncGoogleAds(userId: number, accessToken: string): Promise<void> {
  ...
  for (const resource of resourceNames.slice(0, 10)) {
    ...
    const data = (await searchRes.json()) as {
      results?: Array<{ campaign: Record<string, unknown>; metrics: Record<string, unknown> }>;
    };
    ...
    for (const row of data.results ?? []) {
      ...
      const cpc = Number(m["averageCpc"] ?? 0) / 1_000_000;

      const statusMap: Record<string, "active" | "paused" | "ended" | "draft"> = {
        ENABLED: "active",
        PAUSED: "paused",
        REMOVED: "ended",
      };
...
async function syncMetaAds(userId: number, accessToken: string): Promise<void> {
  ...
  for (const account of (accountsData.data ?? []).slice(0, 5)) {
    ...
    for (const c of campaignsData.data ?? []) {
      const insights =
        ((c["insights"] as Record<string, unknown>)?.["data"] as Record<string, unknown>[])?.[0] ??
        {};
  ...
  for (const account of (accountsData.elements ?? []).slice(0, 5)) {
    ...
    for (const c of campaignsData.elements ?? []) {
      ...
      const analyticsData = analyticsRes.ok
        ? ((await analyticsRes.json()) as { elements: Array<Record<string, unknown>> })
        : { elements: [] };
      ...
      const statusStr = String(
        ((c["status"] as Record<string, unknown>)?.["status"] as string) ?? "ACTIVE",
      );
      const statusMap: Record<string, "active" | "paused" | "ended" | "draft"> = {
        ACTIVE: "active",
        PAUSED: "paused",
      ...
      const budget = Number(
        ((c["totalBudget"] as Record<string, unknown>)?.["amount"] as number) ?? 0,
      );
...
async function syncMicrosoftAds(userId: number, accessToken: string): Promise<void> {
  ...
  // Microsoft Ads uses SOAP; the REST endpoints are limited.
  // Log and skip — implementation requires the Microsoft Advertising SDK or SOAP client.
  void userId;
  void accessToken;
  throw new Error(
    "Microsoft Ads sync requires the SOAP API. Use the microsoft-bingads Node SDK for full implementation.",
  );
}
